...
function createBoard(numFilas, numColumnas){
    ...
    for (let i = 0; i < numFilas;  i++) {
        ...
        for(let j = 0; j < numColumnas; j++){
            ...
            casilla.push(dataCasilla)//push: agrega el elemnto 0 o un mono
        }
        rows.push(casilla)//push: agrega el elemnto casilla
    }
    return rows
}
...
function ocultra(casilla1, casilla2){
    casilla1.visible = false
    casilla2.visible = false
}
...
function crearDivColumna(fil, col, simbolo, visible){
    ...
    //Pone invisible los emoji
    if(visible == true){
        divBut.innerText = simbolo
    }else {
        divBut.innerText = ""
    }
    divBut.onclick = buttOnClik;// No estoy ejecutando la funcion "buttOnClik()"-->NO
